...
export const checkConnectionStatus = async (userId) => {
  return newrelic.startSegment("github_service:checkStatus", true, async () => {
  ...
  return newrelic.startSegment("github_service:fetchRepos", true, async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://api.github.com/user/repos?visibility=public&affiliation=owner,collaborator&sort=updated&per_page=100",
        {
          headers: {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          throw new AppError(
            "GitHub token expired or revoked. Please reconnect.",
            401,
          );
        }
        throw new Error(
