...
const { combine, timestamp, printf, colorize, json, errors, splat } =
  winston.format;
...
const DEV_FORMAT = combine(
  ...
  splat(),
  printf(
    ({ timestamp, level, message, stack, service, environment, ...meta }) => {
...
const buildProdFormat = (nrEnricher) => {
  ...
  if (nrEnricher) formatters.push(nrEnricher()); // Invoke to get the format instance
...
if (IS_PROD) {
  try {
    const { default: newrelicFormatter } =
      await import("@newrelic/winston-enricher");
